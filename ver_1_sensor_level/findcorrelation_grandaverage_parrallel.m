...
for i = 1:length(inputsarray) % for each word
    ...
    for j = 1:306
        ...
        for k = 1:length(keeptrueSig)
            ...
            average = mean(MEGWORD.avg(j,((channeltimepos+latency)-(ceil(window/2))+1):((channeltimepos+latency-1)+(ceil(window/2)))));
            averagechannelsig = zeros(1,length(keeptrueSig));
    ...
    for i = 1:length(inputsarray) % for each word
        ...
        for j = 1:306
            ...
            for k = 1:length(keeprandSig)
                channeltimepos = find(MEGWORD.time(1,:) == keepSigtime(k));
                average = mean(MEGWORD.avg(j,((channeltimepos+latency)-(ceil(window/2))+1):((channeltimepos+latency-1)+(ceil(window/2)))));
                averagechannelsig = zeros(1,length(keeprandSig));
                averagechannelsig(k) = average;
            end
...
%col = pink(128); %   tmp = linspace(0,1,200)';
...
%cfg.contournum      = 0;                   %(default = 6), see CONTOUR
