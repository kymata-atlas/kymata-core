...
for i = 1:length(words) % for each word
    ...
    trueSigarray = repmat(trueSig(~isnan(trueSig)), numsensors, 1);
    trueSensorArray= MEGdata{1}(trueSig(~isnan(trueSig));
    ALL = [trueSigarray;trueSensorArray] 
    coeff = coeff(trueSensorArray, trueSigarray)
    coeff = deleate evencolums;
    coeff = deleat odd rows;
    truewords = diag(coeff)% possibly quicker way ala Russel (cov(truesig(sensor),truesig(array)))/(standarddevx*standarddevY)
    ...
    R = sum((R-repmat(mean(R),r,1)).*repmat(M-mean(M),1,c))./((std(R).*std(M)).*(r-1));
...
cfg.gridscale       = 200;                  % scaling grid size (default = 67)
