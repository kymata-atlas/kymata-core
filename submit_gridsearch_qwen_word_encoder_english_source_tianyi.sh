...
#SBATCH --output=kymata-core-data/output/qwen_english/source/encoder/log/tianyi/slurm_log_%a.txt
#SBATCH --error=kymata-core-data/output/qwen_english/source/encoder/log/tianyi/slurm_log_%a.txt
#SBATCH --ntasks=1
...
echo "Reading neuron list from npy (subrange Tianyi: [0, 9554))"

n_jobs=36

# Single-pass extraction (loads the npy once).
out=$(python kymata/invokers/read_npy.py "$npy_file" --range 0 9554 --format bash)
