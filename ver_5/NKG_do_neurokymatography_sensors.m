...
nVertices = 403; %2562                                                              % number of sensors
...
for w = 1:numel(itemlist) 
    ...
    inputfilename = [rootCodeOutputPath version '/' experimentName, '/3-sensor-data/fif-out-averaged-GRADS+MAGS-readyforNKG/', thisword, '.fif'];

    disp(num2str(w));
    if (exist(inputfilename, 'file'))
        sensordata = fiff_read_evoked([rootCodeOutputPath version '/' experimentName, '/3-sensor-data/fif-out-averaged-GRADS+MAGS-readyforNKG/', thisword, '.fif']);
        eval([ 'grandaverage_', thisword ' = sensordata.evoked.epochs;' ]);
    end
end

% Import output template stc
sensordata = fiff_read_evoked([rootCodeOutputPath version '/' experimentName, '/3-sensor-data/fif-out-averaged-GRADS+MAGS-readyforNKG/item1.fif']);
...
wordsignalstrue = zeros(numel(itemlist), size(stimulisig.bluecomponentSplit, 2));

for w = 1:numel(itemlist) 
        thisword = char(itemlist(w));
        wordsignalstrue(w,:) = stimulisig.bluecomponentSplit(strcmp(thisword, stimulisig.name),:);
end
