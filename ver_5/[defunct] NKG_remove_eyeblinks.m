...
addpath /imaging/at03/NKG_Code/Version5/ICA_functions/
addpath /imaging/at03/meg-pd-1.2-11.x86_64/opt/neuromag/meg_pd_1.2/
...
for p = 1:numel(participentIDlist)
    ...
    for b = 1:participentNumBlockHash.get(char(participentIDlist(p)))
        ...
        fname = [rootCodeOutputPath, version, '/', experimentName, '/1-preprosessing/sss/', participentIDlist{p} '_nkg_part' num2str(b) '_raw_sss_movecomp.fif'];
        ...
        clear S D
        S.Fdata      = fname;
        S.Pout       = [rootCodeOutputPath, version '/' experimentName, '/2-do-ICA/1-outputSPMfiles/', fstem, '-SPM.mat'];
        ...
        S.conds      = 0;
        S.HPIfile    = [rootDataSetPath, experimentName, '/', participentIDlist{p} '/' participentIDlist{p} '_nkg_part' num2str(b) '_raw.fif']; 
        S.grms       = 0;
        ...
        S.Fdata      = fname;
        S.D          = [rootCodeOutputPath, version '/' experimentName, '/2-do-ICA/1-outputSPMfiles/', fstem, '-SPM.mat'];       % structure for splitting
...
for p = 1:numel(participentIDlist)
    ...
    clear S D
    badchanfilename = [rootDataSetPath, experimentName, '/', participentIDlist{p} '/EEG_bad_channels.txt'];
    ...
    disp(['badchannels:' badchannels]);
  
    for s = 1:participentNumBlockHash.get(char(participentIDlist(p)))

        fstem = [char(participentIDlist(p)), '_nkg_part'  num2str(s),  '_raw_sss_movecomp'];

        load ([rootCodeOutputPath, version '/' experimentName, '/2-do-ICA/1-outputSPMfiles/', fstem, '-SPM-eeg.mat']);
        ...
        save([rootCodeOutputPath, version '/' experimentName, '/2-do-ICA/1-outputSPMfiles/', fstem, '-SPM-eeg-withbadchannelsmarked.mat'], 'D');
...
cd([rootCodeOutputPath, version '/' experimentName, '/2-do-ICA/1-outputSPMfiles']);

% set up queue 
workerpath={'/imaging/at03/NKG_Code/Version5/ICA_functions'};
...
for p = 1:numel(participentIDlist)
    ...
    for b = 1:participentNumBlockHash.get(char(participentIDlist(p)))
        ...
        for type = {'mags' 'grds' 'eeg-withbadchannelsmarked'}
    
        
            rawname = [rootCodeOutputPath, version, '/', experimentName, '/1-preprosessing/sss/', participentIDlist{p} '_nkg_part' num2str(b) '_raw_sss_movecomp.fif'];
            [remove fstem ext] = fileparts(rawname);
            
            clear S D
            S.D       = [rootCodeOutputPath, version '/' experimentName, '/2-do-ICA/1-outputSPMfiles/', fstem, '-SPM-' type{1} '.mat'];
            S.samppct = 1;
            if (strcmp(type{1}, 'eeg-withbadchannelsmarked'))
                S.excbad  = 1;
            else
                S.excbad  = 0;
            end
            S.newpath = [rootCodeOutputPath, version '/' experimentName, '/2-do-ICA/1-outputSPMfiles/'];
            ...
            scheduler=cbu_scheduler('custom',{'compute', 12, 5, 43200});   %Standard
...
for p = 1:numel(participentIDlist)
    ...
    for s = 1:participentNumBlockHash.get(char(participentIDlist(p)))
        ...
        if exist([rootDataSetPath, experimentName,'/', char(participentIDlist(p)) '/EMEG_bad_channels_fif_names.txt'], 'file')

            unixCommand   = ['mne_mark_bad_channels '];
            unixCommand   = [unixCommand ' --bad  ' rootDataSetPath, experimentName,'/', char(participentIDlist(p)) '/EMEG_bad_channels_fif_names.txt '];
            unixCommand   = [unixCommand rootCodeOutputPath, version, '/', experimentName, '/1-preprosessing/sss/', participentIDlist{p} '_nkg_part' num2str(s) '_raw_sss_movecomp.fif'];
