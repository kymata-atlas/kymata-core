...
cutoff = 990;                                                                       % cutt-off for signal times
nWords = 400;                                                                       % number of words
nTimePoints = 2001; %1518;                                                                 % number of time points in raw data
nVertices = 376; %2562                                                              % number of sensors
...
for w = 1:numel(itemlist) 
    ...
    inputfilename = [rootCodeOutputPath version '/' experimentName, '/3-sensor-data/fif-out-averaged-GRADS+MAGS-readyforNKG/visual/', thisword, '.fif'];

    disp(num2str(w));
    if (exist(inputfilename, 'file'))
        sensordata = fiff_read_evoked([rootCodeOutputPath version '/' experimentName, '/3-sensor-data/fif-out-averaged-GRADS+MAGS-readyforNKG/visual/', thisword, '.fif']);
        eval([ 'grandaverage_', thisword ' = sensordata.evoked.epochs;' ]);
    end
end

% Import output template stc
sensordata = fiff_read_evoked([rootCodeOutputPath version '/' experimentName, '/3-sensor-data/fif-out-averaged-GRADS+MAGS-readyforNKG/visual/item1.fif']);
...
wordsignalstrue = zeros(numel(itemlist), size(stimulisig.horizontalpositionSplit, 2));

for w = 1:numel(itemlist) 
        thisword = char(itemlist(w));
        wordsignalstrue(w,:) = stimulisig.horizontalpositionSplit(strcmp(thisword, stimulisig.name),:);
end
