        ...
        for pair in hexel_coordinate_pairs:
            ...
            if pair == [(0, 0), (20, 0)]:
                expected_paths.append(
                    [np.linspace(0, 20, 100, endpoint=True), np.array([0] * 100)]
                )
            else:
                ctr_points = plotter._make_bspline_ctr_points(pair)
                expected_paths.append(plotter._make_bspline_path(ctr_points))

        bspline_paths_rounded = [
            round(x_ii, 2) for x in bspline_paths for x_i in x for x_ii in x_i
        ]
        expected_paths_rounded = [
            round(x_ii, 2) for x in expected_paths for x_i in x for x_ii in x_i
        ]
