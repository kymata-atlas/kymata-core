...
catch plotraw=0;   % CW changed to 0: plotraw=1;
end
...
if strcmpi(domode,'classify') | strcmpi(domode,'both') | strcmpi(domode,'view')
	...
	% Plot options:
	try plotacts=S.plotacts;  catch plotacts=0; end   % CW changed to 0: plotacts=1;
	try plottopo=S.plottopo;  catch plottopo=1; end	
	...
	if plotacts
		...
		[afig achfig acts]=meg_plotcnt(E);
