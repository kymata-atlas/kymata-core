...
for p = 1:numel(participentIDlist) 
    ...
    for w = 1:numel(wordlist)
        ...
        thisword = char(wordlist(w));
            ...
            for s = 1:participentSessionHash.get(char(participentIDlist(p)))
                unixCommand = [unixCommand '--ave  ', rootCodeOutputPath, version '/' experimentName, '/2-sensor-data/aveFiles/', char(participentIDlist(p)) '-' thisword '.ave '];
            end
               
            unixCommand = [unixCommand '--gave  ', rootCodeOutputPath, version '/' experimentName, '/2-sensor-data/fif-out-averaged/', char(participentIDlist(p)) '-' thisword '.fif '];
...
for p = 1:numel(participentIDlist)
            ...
            for s = 1:participentSessionHash.get(char(participentIDlist(p)))
                unixCommand = [unixCommand '--cov ', rootCodeOutputPath, version '/' experimentName, '/2-sensor-data/cov-description-files/', char(participentIDlist(p)) '_part' num2str(s) '.cov '];
            end
            
            unixCommand = [unixCommand '--gcov ', rootCodeOutputPath, version '/' experimentName, '/2-sensor-data/averaged-noise-covarience-files/', char(participentIDlist(p)) '_gcov.fif '];
...
for p = 1:numel(participentIDlist) 
  ...
  filein = [ rootCodeOutputPath, version '/' experimentName, '/2-sensor-data/averaged-noise-covarience-files/', char(participentIDlist(p)), '_gcov.fif'];    % input filename of covariance matrix file
  ...
  saveas(h, [rootCodeOutputPath, version '/' experimentName, '/2-sensor-data/noise_covarience_plots/' char(participentIDlist(p)) '_gcov_plot' ], 'jpg');
