...
function NKG_do_neurokymatography_standard_RandomEffectsTest(subject, leftright, functionname, functionlocation, stimulisigFunctionName, cutoff, nWords, nTimePoints, nVertices, outputfolder, inputfolder)
...
outputSTC = mne_read_stc_file([rootCodeOutputPath version '/' experimentName, '/3-single-trial-source-data/vert2562-smooth5-nodepth-corFM-snr1-signed/meg10_0003-ached-lh.stc']);
...
for w = 1:numel(wordlist) 
    ...
    inputfilename = [rootCodeOutputPath version '/' experimentName, '/3-single-trial-source-data/',inputfolder, '/', subject '-' thisword '-' leftright '.stc'];
...
cd(fullfile(rootCodeOutputPath, version, experimentName, '5-backedup-mat-files', outputfolder, '/randomeffeects/'));
save('-v7.3',[subject '_' functionname '_' leftright '_' num2str(nVertices) 'verts_-200-800ms_cuttoff' num2str(cutoff) '_' num2str(numberOfIterations) 'perms_ranksumpval_NegitiveIsZero_snr1_nodepth'], 'outputSTC', 'functionname', 'leftright', 'latencies', 'latency_step', 'length_of_longest_stimuli', 'nTimePoints', 'nVertices', 'nWords', 'numberOfIterations');
