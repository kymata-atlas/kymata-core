...
function NKG_do_neurokymatography_standard_SnPM(seed,leftright, functionname, functionlocation, stimulisigFunctionName, cutoff, nWords, nTimePoints, nVertices, outputfolder, inputfolder)
...
outputSTC = mne_read_stc_file([rootCodeOutputPath, version, '/', experimentName, '/4-averaged-by-trial-data/', inputfolder,'/', char(wordlist(1)), '-', leftright, '.stc']);
...
load([rootFunctionPath experimentName functionlocation 'stimulisig.mat']);
...
%scramble words
rng('shuffle');
wordsignalstrue = wordsignalstrue(randperm(size(wordsignalstrue,1)),:);
...
for w = 1:numel(wordlist) 
    ...
    inputfilename = [rootCodeOutputPath version '/' experimentName, '/4-averaged-by-trial-data/',inputfolder, '/', thisword '-' leftright '.stc'];
...
cd(fullfile(rootCodeOutputPath, version, experimentName, '5-backedup-mat-files', outputfolder, '/SnPM/'));
save('-v7.3',['seed' num2str(seed) '_' functionname '_' leftright '_' num2str(nVertices) 'verts_-200-800ms_cuttoff' num2str(cutoff) '_' num2str(numberOfIterations) 'perms_ranksumpval_NegitiveIsZero_snr1_nodepth'], 'outputSTC', 'functionname', 'leftright', 'latencies', 'latency_step', 'length_of_longest_stimuli', 'nTimePoints', 'nVertices', 'nWords', 'numberOfIterations');
