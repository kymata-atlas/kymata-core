...
cutoff = 600;                                                                      % cutt-off for signal times
nWords = 480;                                                                       % number of words
nTimePoints = 2068;                                                                 % number of time points in raw data
nVertices = 10242;                                                                   % number of sensors
leftright = 'lh';                                                                   % hemisphere
numberOfIterations = 5; %30 = 
f_alpha = 0.001;
defaultdownsamplerate = 5;
functionname = 'autocorrr pitch';
reconstructionAndEncodingFolder = 'vert10242-smooth5-nodepth-corFM-snr1-signedwithminusequalszero'


% Initialise=============================================================

% Import output template stc
outputSTC = mne_read_stc_file([rootCodeOutputPath version '/' experimentName, '/4-averaged-by-trial-data/' reconstructionAndEncodingFolder, '/' char(wordlist(1)) '-' leftright '.stc']);
...
wordsignalstrue = zeros(numel(wordlist), size(stimulisig.f0autocorr_threefive, 2));

for w = 1:numel(wordlist) 
        thisword = char(wordlist(w));
        wordsignalstrue(w,:) = stimulisig.f0autocorr_threefive(strcmp(thisword, stimulisig.name),:);
end
...
for w = 1:numel(wordlist) 
    ...
    inputfilename = [rootCodeOutputPath version '/' experimentName, '/4-averaged-by-trial-data/', reconstructionAndEncodingFolder, '/', thisword '-' leftright '.stc'];
...
for q = 1:length(latencies);
    ...
    allcorrs(allcorrs<-0.9999999) = -0.9999999; % gets rid of (potential) rounding problem with log generating complex numbers in the next line
