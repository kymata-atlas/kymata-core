...
% Global variables=======================================================
latencies = -30:5:200;% -30:5:500;                                                  % what latencies
cutoff = 900;                                                                       % cutt-off for signal times
nWords = 480;                                                                       % number of words
nTimePoints = 1518;                                                                 % number of time points in raw data
...
outputSTC = mne_read_stc_file([rootCodeOutputPath 'Version4_source_level_CUDA/' experimentName, '/3-averaged-by-trial-data/averagemesh-vert642-smooth5-elisinvsol/' char(wordlist(1)) '-' leftright '.stc']);
...
for w = 1:numel(wordlist) 
    ...
    inputfilename = [rootCodeOutputPath 'Version4_source_level_CUDA/' experimentName, '/3-averaged-by-trial-data/averagemesh-vert642-smooth5-elisinvsol/', thisword '-' leftright '.stc'];
...
% % Import output template stc
% outputSTC = mne_read_stc_file([rootCodeOutputPath 'Version4_source_level_CUDA/' experimentName, '/2-single-trial-source-data/averagemesh-vert642-smooth5-elisinvsol/meg10_0009-', char(wordlist(1)) '-' leftright '.stc']);
% outputSTC = rmfield(outputSTC, 'data');
% outputSTC.data = zeros(nTimePoints, nVertices);
% 
% % Import source data
...
%      thisword = char(wordlist(w));
%      
%      inputfilename = [rootCodeOutputPath 'Version4_source_level_CUDA/' experimentName, '/2-single-trial-source-data/averagemesh-vert642-smooth5-elisinvsol/meg10_0009-', thisword '-' leftright '.stc'];
%  
%      disp(num2str(w));
%      if (exist(inputfilename, 'file'))
%          sensordata = mne_read_stc_file(inputfilename);
...
wordsignalstrue = zeros(numel(wordlist), size(stimulisig.C0, 2));

tic;

for w = 1:numel(wordlist) 
        thisword = char(wordlist(w));
        wordsignalstrue(w,:) = stimulisig.C0(strcmp(thisword, stimulisig.name),:);
end
...
for q = 1:length(latencies);
    ...
    R = nansum((bsxfun(@minus,MEGdata,nanmean(MEGdata, 1)).*bsxfun(@minus,wordsignalstemp,nanmean(wordsignalstemp, 1))), 1)./((sqrt(nansum(bxsfun(@minus,MEGdata,nanmean(MEGdata, 1)).^2, 1))).*(sqrt(nansum(bxsfun(@minus,wordsignalstemp,nanmean(wordsignalstemp, 1)).^2, 1))));
...
outputSTC.tmin = -0.1;
mne_write_stc_file( [rootCodeOutputPath 'Version4_source_level_CUDA/' experimentName, '/5-neurokymography-output/averagemesh-vert642-smooth5-elisinvsol/test-' leftright '.stc'], outputSTC);
