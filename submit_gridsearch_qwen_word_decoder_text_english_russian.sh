...
#SBATCH --output=kymata-core-data/output/qwen_english_russian/sensor/all/decoder_text/log/slurm_log_%a.txt
#SBATCH --error=kymata-core-data/output/qwen_english_russian/sensor/all/decoder_text/log/slurm_log_%a.txt
#SBATCH --ntasks=1
...
"$VENV_PYTHON" kymata/invokers/run_gridsearch.py \
  ...
  --mfa True \
  --n-splits 400 \
  --save-plot-location "/imaging/projects/cbu/kymata/analyses/tianyi/russian-english/kymata-core/kymata-core-data/output/qwen_english_russian/sensor/all/decoder_text/expression/${layer_num[$SLURM_ARRAY_TASK_ID]}" \
  --save-expression-set-location "/imaging/projects/cbu/kymata/analyses/tianyi/russian-english/kymata-core/kymata-core-data/output/qwen_english_russian/sensor/all/decoder_text/expression/${layer_num[$SLURM_ARRAY_TASK_ID]}" \
  --overwrite
