...
def asr_models_loop_full():
    ...
    log_dir = '/imaging/projects/cbu/kymata/analyses/tianyi/kymata-toolbox/log_asr_models_full/'
    ...
    _lats = np.array([lat_sig[0, j, :] for j in range(lat_sig.shape[1]) if (lat_sig[0, j, 0] != 0 and lat_sig[0, j, 3] > 20)])
    stds.append(np.std(_lats[:, 0]))

    ax.scatter(_lats[:, 0], _lats[:, 4], marker='x')
    ...
    # plt.legend()
    # plt.xlim(-10, 60)
    plt.savefig('asr_models_testing_full.png')
