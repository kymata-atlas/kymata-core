...
def main():
    ...
    # Transforms
    parser.add_argument('--input-stream', type=str, required=True, choices=["auditory", "visual", "tactile"], help="The input stream for the transforms being tested.")
    ...
    parser.add_argument('--replace-nans', type=str, required=False, choices=["zero", "mean"], default=None, help="If the transform contour contains NaN values, this will replace them with the specified values.")
    ...
    if args.phonetics:
        ...
        _logger.info(f"Functions to be tested: {args.transform_name}")
    ...
    if args.asr_option == 'all' or 'linguistics' in args.transform_path:
        ...
        fig = expression_plot(
            ...
            save_to=Path(args.save_plot_location, function_values.name + '_gridsearch.png'),
            overwrite=args.overwrite,
            show_legend=True,
        )
        pyplot.close(fig)
        ...
        for num, transform_name in enumerate(args.transform_name):
            ...
            fig = expression_plot(
                es,
                paired_axes=channel_space == "source",
                save_to=Path(args.save_plot_location, function_values.name + f'_{nn_i}' + '_gridsearch.png'),
                overwrite=args.overwrite,
                show_legend=True,
            )
            pyplot.close(fig)
