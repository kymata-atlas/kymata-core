...
def main():
    ...
    if transform_family_type == 'simple':
        ...
        #     sem_density_name, Preposition_name, Number_name, Noun_name, Determiner_name, Conjunction_name, 
        #     Adverb_name, Adjective_name, Verb_name, Pronoun_name, num_open_node_name, num_close_node_name, 
        #     sentence_end_name, current_depth_name, linear_order_name
        # ]

        # phone_feats_name = [
        #     AA, AE, AH, AO, AW, AY, B, CH, D, DH, EH, ER, EY, F, G, HH, IH, IY, JH, K, L, M, N, NG, OW, OY, P, R, 
        #     S, SH, T, TH, UH, UW, V, W, Y, Z, ZH,
        #     Consonantal, Sonorant, Voiced, Nasal, Plosive, 
        #     Fricative, Approximant, Labial, Coronal, Dorsal, High, Mid, Low, Front, Central, Back, Round, Tense
        # ]

        # word_feats_name_string = [
        ...
        # phone_feats_name_string = [
        #     'AA', 'AE', 'AH', 'AO', 'AW', 'AY', 'B', 'CH', 'D', 'DH', 'EH', 'ER', 
        #     'EY', 'F', 'G', 'HH', 'IH', 'IY', 'JH', 'K', 'L', 'M', 'N', 'NG', 'OW', 'OY', 
        #     'P', 'R', 'S', 'SH', 'T', 'TH', 'UH', 'UW', 'V', 'W', 'Y', 'Z', 'ZH',
        #     'Consonantal', 'Sonorant', 'Voiced', 'Nasal', 'Plosive', 'Fricative', 'Approximant', 'Labial', 
        #     'Coronal', 'Dorsal', 'High', 'Mid', 'Low', 'Front', 'Central', 'Back', 'Round', 
        #     'Tense'
        # ]

        word_feats_name = [
            log_freq_name, freq_phon_neighbour_name, sem_neighbour_density_name, 
            sem_density_name, Number_name, Noun_name, 
            Adverb_name, 
            current_depth_name, linear_order_name
        ]

        phone_feats_name = [
            Consonantal, Sonorant, Voiced, Nasal, 
            Fricative, Labial, Coronal, High, Mid, Front, Central, Back, Tense,
            AE, AO, AY, B, DH, EH, ER, EY, HH, IH, IY, K, L, M, N, NG, OW, OY, 
            S, SH, UH, V, W, Y, Z, ZH
        ]
        ...
        count_non_empty = 0
        for i, feature_name in enumerate(word_feats_name):
            if feature_name != []:
                count_non_empty += 1
        ...
        for i, feature_name in enumerate(word_feats_name):
            if feature_name != []:
                for j in feature_name:
                    word_colour[j] = to_hex((0.4 + 0.6 * index_non_empty/count_non_empty, 0, 0.6 - 0.6 * index_non_empty/count_non_empty))
                    word_disp[j] = word_feats_name_string[i]
                index_non_empty += 1
        ...
        count_non_empty = 0
        for i, feature_name in enumerate(phone_feats_name):
            if feature_name != []:
                count_non_empty += 1
        ...
        for i, feature_name in enumerate(phone_feats_name):
            if feature_name != []:
                for j in feature_name:
                    phone_colour[j] = to_hex((0, 0.4 + 0.6 * index_non_empty/count_non_empty, 0.6 - 0.6 * index_non_empty/count_non_empty))
                    phone_disp[j] = phone_feats_name_string[i]
                index_non_empty += 1
        ...
        fig = expression_plot(expression_data_tvl + expression_data_word_manual + expression_data_phone_manual + 
                ...
                paired_axes=True, minimap='large', show_legend=True, 
                # show_only=[item for sublist in word_feats_name for item in sublist], 
                show_only=AE + AO + AY + B + DH + EH + ER + EY + HH + IH + IY + K + L + M + N + NG + OW + OY + S + SH + UH + V + W + Y + Z + ZH,
                ylim=-125,
                color=word_colour | phone_colour,
                legend_display=word_disp | phone_disp)
        fig.savefig(f"/imaging/projects/cbu/kymata/analyses/tianyi/kymata-core/kymata-core-data/output/paper/figure_3_art_new.png")
