...
def build_graph_dict(hexels: Dict[str, IPPMHexel],
    ...
    sorted_ = _sort_by_latency(hexels, hemi, list(function_hier.keys()))
    ...
    while len(hier.keys()) > 0:
        top_level = _get_top_level_functions(hier) # get function that is a child, i.e., it doesn't have any arrows going out of it
        for f in top_level:
            # We do the following:
            # if f == input_node:
            #     default_settings
            ...
            if f in inputs:
                ...
                best_pairings = (
                        sorted_[f].left_best_pairings
                        if hemi == 'leftHemisphere' else
                        sorted_[f].right_best_pairings
                    )
                ...
                for parent in parents:
                    ...
                    if parent in inputs:
                        ...
                        graph[f + '-0'].inc_edges.append(parent)
                    else:
                        parent_pairings = (
                            sorted_[parent].left_best_pairings if hemi == 'leftHemisphere' else
                                sorted_[parent].right_best_pairings
                            )
                        ...
                        graph[f + '-0'].inc_edges.append(parent + '-' + str(len(parent_pairings) - 1))
    return graph


def _get_top_level_functions(hier: Dict[str, List[str]]) -> set:
    """
