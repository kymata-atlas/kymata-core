...
class IPPMBuilder(object):
    ...
    def build_graph(
        ...
        while len(hier.keys()) > 0:
            top_level = self._get_top_level_functions(
                hier
            )  # get function that is a child, i.e., it doesn't have any arrows going out of it
            for f in top_level:
                # We do the following:
                # if f == input_node:
                #     default_settings
                ...
                if f in inputs:
                    ...
                    hier.pop(
                        f
                    )  # make sure to pop so we don't get the same top level fs every loop.

                    best_pairings = (
                        sorted[f].left_best_pairings
                        if hemi == "leftHemisphere"
                        else sorted[f].right_best_pairings
                    )
                    ...
                    for idx, pair in enumerate(best_pairings):
                        ...
                        graph[f + "-" + str(idx)] = Node(
                            -10 * np.log10(magnitude),
                    ...
                    for parent in parents:
                        ...
                        if parent in inputs:
                            ...
                            parent_pairings = (
                                sorted[parent].left_best_pairings
                                if hemi == "leftHemisphere"
    ...
    def _sort_by_latency(
        self, hexels: Dict[str, IPPMHexel], hemi: str, functions: List[str]
    ) -> Dict[str, IPPMHexel]:
        """
