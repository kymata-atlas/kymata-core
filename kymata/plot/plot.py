...
def _hexel_minimap_data(expression_set: HexelExpressionSet, alpha_logp: float, show_functions: list[str]) -> tuple[NDArray, NDArray]:
    ...
    expression_set : HexelExpressionSet
        The set of hexel expressions to analyze.
    alpha_logp : float
        The logarithm of the p-value threshold for significance. Hexels with values below this threshold
        are considered significant.
    show_functions : list[str]
        A list of function names to consider for significance. Only these functions will be checked
        for significant hexels.

    Returns:
    --------
    tuple[NDArray, NDArray]
        A tuple containing two arrays (one for the left hemisphere and one for the right hemisphere).
        Each array has a length equal to the number of hexels in the respective hemisphere, with entries:
        - 0, if no function is ever significant for this hexel.
        - i + 1, where i is the index of the function (from show_functions) that is significant for this hexel.
    ...
    indicates whether the hexel is significant for any function and, if so, which function it is significant for.
    """
...
def expression_plot(
    ...
    -----------
    expression_set : ExpressionSet
        The set of expressions to plot, containing functions and associated data.
    show_only : Optional[str | Sequence[str]], optional
        A string or a sequence of strings specifying which functions to plot.
        If None, all functions in the expression_set will be plotted. Default is None.
    paired_axes : bool, optional
        When True, shows the expression plot split into left and right axes.
    ...
    color : Optional[str | dict[str, str] | list[str]], optional
        Color settings for the plot. Can be a single color, a dictionary mapping function names
        to colors, or a list of colors. Default is None.
    ylim : Optional[float], optional
        The y-axis limit. If None, it will be determined automatically. Default is None.
    xlims : tuple[Optional[float], Optional[float]], optional
        The x-axis limits as a tuple. None to use default values, or set either entry
        to None to use the default for that value. Default is (-100, 800).
    hidden_functions_in_legend : bool, optional
        If True, includes non-plotted functions in the legend. Default is True.
    minimap : bool, optional
        If True, displays a minimap of the expression data. Default is False.
    minimap_view : str, optional
        The view type for the minimap, either "lateral" or other specified views. Default is "lateral".
    minimap_surface : str, optional
        The surface type for the minimap, such as "inflated". Default is "inflated".
    save_to : Optional[Path], optional
    ...
    legend_display : dict[str, str] | None, optional
        Allows grouping of multiple functions under the same legend item. Provide a dictionary
        mapping true function names to display names. Default is None.
...
def plot_top_five_channels_of_gridsearch(
    ...
    function : Function
        The function object whose name attribute will be used in the plot title.
    n_samples_per_split : int
    ...
    auto_corrs : NDArray[any]
        Auto-correlation values array used for plotting the function auto-correlation.
    log_pvalues : any
