...
class TVLDeltaInsTC1LoudnessOnlyDataset(SampleExpressionDataset):
    def __init__(self, data_root: Optional[PathType] = None, download: bool = True):
        ...
        super().__init__(
            name=name,
            filenames=["TVL_2020_delta_ins_tontop_chan1_loudness_only.nkg"],
            data_root=data_root,
            remote_root="https://kymata.org/assets_kymata_toolbox_tutorial_data/gridsearch-result-data/",
            download=download,
        )
...
class TVLDeltaInsTC1LoudnessOnlySensorsDataset(SampleExpressionDataset):
    def __init__(self, data_root: Optional[PathType] = None, download: bool = True):
        ...
        super().__init__(
            name=name,
            filenames=["TVL_2020_delta_ins_tontop_chan1_loudness_only_sensors.nkg"],
            data_root=data_root,
            remote_root="https://kymata.org/assets_kymata_toolbox_tutorial_data/gridsearch-result-data/",
            download=download,
        )
...
def delete_dataset(local_dataset: SampleDataset):
    ...
    assert (
        data_root_path() in local_dataset.path.parents
    ), "Cannot delete dataset outside of data root directory"
