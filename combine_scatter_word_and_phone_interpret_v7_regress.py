...
def asr_models_loop_full():
    ...
    log_dir = f'/imaging/projects/cbu/kymata/analyses/tianyi/kymata-core/kymata-core-data/output/first_speech_paper/salmonn_7b_phone/fc2/log/'
    compare_log_dir = f'/imaging/projects/cbu/kymata/analyses/tianyi/kymata-core/kymata-core-data/output/first_speech_paper/size/salmonn_7b/fc2/log/'
    tvl_compare_log_dir = '/imaging/projects/cbu/kymata/analyses/tianyi/kymata-core/kymata-core-data/output/first_speech_paper/size/salmonn_7b/tvl/log/'
    tvl_log_dir = '/imaging/projects/cbu/kymata/analyses/tianyi/kymata-core/kymata-core-data/output/first_speech_paper/salmonn_7b_phone/tvl/log/'
    ...
    if figure_opt == 'phone':
        ...
        # Compute correlation coefficient and plot the line
        if x_data == 'latency':
            all_x = np.concatenate((enhanced[mask_1, 0], emerge[mask_2, 0]))
            all_y = np.concatenate((enhanced[mask_1, 4], emerge[mask_2, 4]))
        ...
        ax.plot(all_x, m * all_x + b, linestyle='dotted', color='blue')

        # Calculate p-value
        _, p_value = pearsonr(all_x, all_y)
        ...
        # Compute correlation coefficient and plot the line
        if x_data == 'latency':
            all_x = np.concatenate((reduced[mask_1, 0], demolish[mask_2, 0]))
            all_y = np.concatenate((reduced[mask_1, 4], demolish[mask_2, 4]))
    ...
    plt.savefig(f'/imaging/projects/cbu/kymata/analyses/tianyi/kymata-core/kymata-core-data/output/first_speech_paper/scatter/salmonn_7b_{figure_opt}_interpret_{thres_feats}_{occur_thres}_{x_data}_v9_regress.png', dpi=600, bbox_inches="tight")
