...
function NKG_do_neurokymatography_standard(leftright, functionname, functionlocation, stimulisigFunctionName, cutoff, nWords, nTimePoints, nVertices, outputfolder, inputfolder, inv)

% setup file for neurokymatography scripts

% Add paths
addpath /imaging/local/linux/mne_2.6.0/mne/matlab/toolbox/;
addpath /imaging/at03/NKG_Code/Version6/mne_matlab_functions/;
...
load([rootFunctionPath experimentName functionlocation 'stimulisig.mat']); % create word-signals true, in same order as wordlist
if strcmp('true', inv) % if 'inverse' required flip stimulisigFunctionName
    eval(['stimulisig.' stimulisigFunctionName ' = stimulisig.' stimulisigFunctionName '.*-1;']);
end
...
for q = 1:length(latencies);
    ...
    % can we do positive and negitive in the same run?
