...
%only print ROIs
labelfilename = ['/imaging/at03/NKG_Data_Sets/DATASET_3-01_visual-and-auditory/mne_subjects_dir/fsaverage/label/ventral-', leftright, '.label'];
%labelfilename = ['/imaging/at03/NKG_Data_Sets/DATASET_3-01_visual-and-auditory/mne_subjects_dir/fsaverage/label/DK_Atlas/pericalcarine-', leftright, '.label'];
fid = fopen(labelfilename);
thisROI = textscan(fid, '%d %f %f %f %f', 'HeaderLines', 2);
q = zeros(numberofverts,1);
for i= 1:length(thisROI{1,1})
    if (thisROI{1,1}(i) < numberofverts-1)
        position = find(thisROI{1,1}(i) == vertexorder);
        q(position) = 1;
    end
end
for i= 1:length(results)
    if (q(i) == 0)
        results(i, :) = ones(1, size(results,2));
    end
end
...
set(gca, 'YScale', 'log', 'XLim', [-50 400], 'YLim', [1E-60 1]);
