...
% Global variables=======================================================
latencies = 30:5:70;% -30:5:500;        50-110 and 600ms in a night                 % what latencies
cutoff = 800;                                                                       % cutt-off for signal times
nWords = 400;                                                                       % number of words
nTimePoints = 1300;                                                                 % number of time points in raw data
...
outputSTC = mne_read_stc_file(['/imaging/at03/NKG/saved_data/source_space/3-averaged-by-trial-data/averaged_ds-642/smooth5/elisabeth_invsol/bashed-' leftright '.stc']);
...
for w = 1:numel(wordlist) 
    ...
    inputfilename = ['/imaging/at03/NKG/saved_data/source_space/3-averaged-by-trial-data/averaged_ds-642/smooth5/elisabeth_invsol/', thisword '-' leftright '.stc'];
...
wordsignalstrue = zeros(numel(wordlist), size(stimulisig.loudnessinstantPhons, 2));

tic;

for w = 1:numel(wordlist) 
        thisword = char(wordlist(w));
        wordsignalstrue(w,:) = stimulisig.loudnessinstantPhons(strcmp(thisword, stimulisig.name),:);
end
...
% create word-signals
[junk permorder] = sort(rand(30,nWords),2); %20 for eight
...
for q = 1:length(latencies);
    ...
    outputSTC.data((latency*downsamplerate)+(pre_stimulus_window*downsamplerate+1),:) = pvalues';
...
for k = 1:size(outputSTC.data, 1)
   for l = 1:size(outputSTC.data, 2)
        if (outputSTC.data(k,l) > 0.06)
           outputSTC.data(k,l) = 1;
        end
        if (outputSTC.data(k,l) <= 0.06 && outputSTC.data(k,l) > 0.04)
            outputSTC.data(k,l) = 0.1;
        end
        if (outputSTC.data(k,l) <= 0.04 && outputSTC.data(k,l) > 0)
...
outputSTC.tmin = -0.1;
mne_write_stc_file( ['/imaging/at03/NKG/saved_data/source_space/4-neurokymatogaphy-output/elisabeth_invsol/test-' leftright '.stc'], outputSTC);
