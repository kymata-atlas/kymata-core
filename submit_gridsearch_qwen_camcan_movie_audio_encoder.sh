...
#SBATCH --output=kymata-core-data/output/camcan_movie/audio_encoder/log/slurm_log_%a.txt
#SBATCH --error=kymata-core-data/output/camcan_movie/audio_encoder/log/slurm_log_%a.txt
#SBATCH --ntasks=1
...
python kymata/invokers/run_gridsearch.py \
  ...
  --plot-top-channels \
  --transform-path '/imaging/projects/cbu/kymata/analyses/tianyi/llm_workspace/activations/camcan_movie/audio_encoder' \
  --transform-name "${layer_num[$SLURM_ARRAY_TASK_ID]}" \
  --n-derangements 5 \
  --asr-option 'all' \
  --num-neurons 1280 \
  --n-splits 480 \
  --save-plot-location "/imaging/projects/cbu/kymata/analyses/tianyi/kymata-core/kymata-core-data/output/camcan_movie/audio_encoder/expression/${layer_num[$SLURM_ARRAY_TASK_ID]}" \
  --save-expression-set-location "/imaging/projects/cbu/kymata/analyses/tianyi/kymata-core/kymata-core-data/output/camcan_movie/audio_encoder/expression/${layer_num[$SLURM_ARRAY_TASK_ID]}" \
  --overwrite
